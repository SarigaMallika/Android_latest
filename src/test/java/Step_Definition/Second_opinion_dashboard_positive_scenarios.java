...
public class Second_opinion_dashboard_positive_scenarios extends Generic_functions {
	...
	public void second_opinion_page() throws Exception {
		try {
			page_wait(50);
			str= driver.findElement(By.xpath(OR_reader("second_opinions_title"))).getText();
			Assert.assertEquals(str,td_reader("second_opinions_title"));
		} catch (Exception e) {
			e.printStackTrace();
			takeScreenShot("second_opinion_positive_dashboard_tc_001");
		}	
	...
	/* TC_002-Validate that the user is able to click on Resume Cases and a pop up will appear */
	...
	public void second_opinion_positive_dashboard_tc_002() throws Exception, InterruptedException{
		try{
			...
			page_wait(30);
			str= driver.findElement(By.xpath(OR_reader("so_resume_case_popup_title"))).getText();
			Assert.assertEquals(str,td_reader("so_resume_case_popup_title"));
			str= driver.findElement(By.xpath(OR_reader("so_resume_case_popup_description"))).getText();
			Assert.assertEquals(str,td_reader("so_resume_case_popup_description"));
	...
	public void Requestedpage() throws Exception {
		try {
			...
			Assert.assertEquals(true,value);
			str= driver.findElement(By.xpath(OR_reader("so_requested_msg"))).getText();
			Assert.assertEquals(str,td_reader("so_requested_msg"));
		} catch (Exception e) {
			e.printStackTrace();
			takeScreenShot("second_opinion_positive_dashboard_tc_003");
		}
	...
	public void Reviewedpage() throws Exception {
		try {
			...
			value = driver.findElement(By.xpath(OR_reader("so_requested_icon"))).isDisplayed();
			Assert.assertEquals(true,value);
			str= driver.findElement(By.xpath(OR_reader("so_reviewed_msg"))).getText();
			Assert.assertEquals(str,td_reader("so_reviewed_msg"));
			close();		
		} catch (Exception e) {
			e.printStackTrace();
			takeScreenShot("second_opinion_positive_dashboard_tc_004");
